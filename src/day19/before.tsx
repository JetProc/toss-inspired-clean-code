...
const Sidebar = ({user, theme}: {user: any; theme: string}) => {
  return (
    <aside className='w-64 border-r h-screen'>
      <nav>메뉴 1</nav>
      <nav>메뉴 2</nav>
      {/* 🚨 Sidebar는 user와 theme이 필요 없지만 UserMenu를 위해 전달 중 */}
      <UserMenu user={user} theme={theme} />
    </aside>
  );
};

const MainLayout = ({user, theme}: {user: any; theme: string}) => {
  return (
    <div className='flex'>
      {/* 🚨 MainLayout 역시 user와 theme이 필요 없지만 Sidebar를 위해 전달 중 */}
...
export const Dashboard = () => {
  ...
  return (
    ...
    <MainLayout user={currentUser} theme={currentTheme} />
  );
};
